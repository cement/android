...
public class KeyChainActivity extends Activity implements KeyChainAliasCallback,OnClickListener{
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.ssl_server_main);
		...
		testSslButton.setOnClickListener(this);
	}
	...
	private void startServer() {
		Intent secureWebServerIntent = new Intent(this, SecureWebServerService.class);
	...
	public void onClick(View v) {
		...
		if(v.equals(serverButton)){
			if (serverButton.getText().equals(getResources().getString(R.string.server_start))) {
		...
		if(v.equals(testSslButton)){
			Intent i = new Intent(Intent.ACTION_VIEW, Uri.parse(TEST_SSL_URL));
			startActivity(i);
		}
		if(v.equals(keyChainButton)){
			installPkcs12();
		}
	}
	
	
	private Thread displayCertificateThread = new Thread(new Runnable() {
		@Override
		public void run() {
	...
	private void displayCertificateInfo() {
		...
		runOnUiThread(new Runnable() {
			...
			public void run() {
				...
				keyChainButton.setVisibility(View.GONE);
